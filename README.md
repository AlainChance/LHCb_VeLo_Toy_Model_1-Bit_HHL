...
In the *XY* projection, these straight lines pass through the origin. As a result, energy hits are expected to have a nearly constant phase in polar coordinates when projected onto the *XY* plane (see Section 3, *Search by triplet — Sort by φ*, in [ALGO-10](https://arxiv.org/pdf/2207.03936)).
...
The class ToleranceEstimator which provides unified statistical estimators for:
      ...
      def find_tracks(self, list_hits):
        ...
        if result is not None:
            ...
            if display_clustering:
                text = f" Refined tol_clusters_est = {tol_clusters_est:.4e} (mean intra-cluster std={mean_std:.4e})"
                line = "-" * (len(text) + 1)
                print(f"\n{line}\n{text}\n{line}")
            ...
            if display_clustering:
                text = " Refined tol_clusters_est: no usable clusters, keeping original tol_clusters"
                line = "-" * (len(text) + 1)
                print(f"\n{line}\n{text}\n{line}")
        ...
        for cluster in clusters:
            ...
            if len(cluster) > threshold:
                ...
                #--------------------------------
                    ...
                    #-------------------------------------------------
                    hit_clusters = self.split_clone_by_direction(
                        track_hits,
                        tol_clone_est,
                    ...
                    #-----------------------------------------------------
                    # Build tracks for each subcluster
                    #-----------------------------------------------------
                    for hit_list in hit_clusters:

                        # Convert Hit objects → array format expected by create_tracks
                        subcluster = np.array([
                            [h.index, h.hit_id, h.x, h.y, h.z, h.module_id, h.theta]
                            for h in hit_list
                        ], dtype=float)
...
It is called by the functions `get_tracks_smart()` and `find_tracks()`. 
...
**Returns**
  - hit_clusters : list[list[Hit]]. A list of subclusters, each representing one physical track. Each subcluster is a list of Hit objects sorted by module_id.    If no clone splitting is needed (e.g., only one valid segment), the function returns a single-element list containing track_hits.
...
```python
    def split_clone_by_direction(self, track_hits, tol_clone_est, display=False):
        """
        ...
        if len(segments) == 0:
            return [track_hits]   # nothing to split
        ...
        #-----------------------------------------
        # Cluster cosines using your 1-D clustering
        #-----------------------------------------
        array_cos = np.array([[i, cos] for i, cos in enumerate(cos_values)])
        seg_clusters = self.cluster_by_last_column(array_cos, tol=tol_clone_est)

        if display:
            print("\n Direction-based segment clusters:")
            for sc in seg_clusters:
                print("  cluster:", sc[:, 0].astype(int))

        #-----------------------------------------
        # Convert segment clusters → hit clusters
        #-----------------------------------------
        hit_clusters = []
        for sc in seg_clusters:
...
[ALGO-11] [Aaij, R., Albrecht, J., Belous, M. et al. Allen: A High-Level Trigger on GPUs for LHCb. Comput Softw Big Sci 4, 7 (2020). https://doi.org/10.1007/s41781-020-00039-7](https://doi.org/10.1007/s41781-020-00039-7)

[ALGO-12] [The CMS Collaboration, Description and performance of track and primary-vertex reconstruction with the CMS tracker, arXiv:1405.6569v2, physics.ins-det, 28 Oct 2014](https://doi.org/10.48550/arXiv.1405.6569)
